...
var SarLibrary_instance; 
  class Library {
    constructor(localStorageKey) {
    this._booksArray = [];
    this.localStorageKey = localStorageKey;
    SarLibrary_instance = this;
  }
 ...
 class addBook(book) {
    ...
    for (var i = 0; i < this._booksArray.length; i++) {
      let bookPicked = this._booksArray[i];
 ...
 class removeBookByTitle(title) {
    for (var i = 0; i < this._booksArray.length; i++) {
      let removedTitle = this._booksArray[i];
  ...
  class removeBooksByAuthor(author) {
    var outcome = false;
 ...
 class getBooksByAuthor(author) {
  ...
  class filterForProperties(bookStringBeingPassed) {
 ...
 class buildTable() {
  ...
  class attachTableToAuthModal(e) {
